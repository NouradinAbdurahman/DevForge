...
| `doctor --release-check` | **Experimental** | New this RC1 finalization pass (`cli/src/core/releaseCheck.js`). The individual check names/messages and the overall PASS/FAIL exit-code contract are unlikely to change, but haven't had a release cycle to prove out yet - left Experimental rather than immediately folded into `doctor`'s own Stable classification above. |
| `rc-validate` | **Experimental** | New this RC1 finalization pass (`scripts/rc-validate.sh`). A release-engineering tool, not a user-facing feature - its section numbering/output format may still change as it gets used for real releases. |
