...
usage() {
    echo "Usage: ./dev profile <list|show|use> [name]"
    exit 1
}
...
case "$1" in
    list)
        ...
        ;;
    show)
        [[ $# -eq 2 ]] || { echo "Usage: ./dev profile show <name>"; exit 1; }
        bf="$(profile_brewfile_path "$2")"
        if [[ ! -f "$bf" ]]; then
            log_error "Unknown profile '$2' - run './dev profile list' to see available profiles"
            exit 1
        fi
        ...
        ;;
    use)
        [[ $# -eq 2 ]] || { echo "Usage: ./dev profile use <name>"; exit 1; }
        bf="$(profile_brewfile_path "$2")"
        if [[ ! -f "$bf" ]]; then
            log_error "Unknown profile '$2' - run './dev profile list' to see available profiles"
            exit 1
        ...
        log_info "Future ./bootstrap.sh / ./dev install runs use it unless overridden with --profile/--minimal/--full."
        ;;
    -h|--help|help)
        usage
        ;;
